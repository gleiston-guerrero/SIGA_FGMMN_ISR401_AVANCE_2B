...
| `verificacion_previa.py`, reparto de firmas | Claude (Anthropic) | El reparto de la verificacion entre firmantes pasa a calcularse desde el historial, archivo por archivo, en lugar de escribirse a mano. El texto anterior afirmaba que Cedeno Avila no tenia confirmaciones en tres carpetas donde si las tiene | Contrastado con `git log --format=%ae -- <archivo>` sobre cada uno de los 973 archivos repartidos |
| Documentos para firmar de `aporte_individual` y `verificacion_previa` | Claude (Anthropic) | Composicion de los PDF desde su `.md`, con las firmas agrupadas en una sola hoja, y armado del PDF depositado: el documento generado mas la hoja firmada que escaneo el equipo | **Las firmas son manuscritas de los tres integrantes.** La hoja escaneada se inserta sin retoque como ultima pagina |
| Etiquetas `2B-final-v5.0` a `2B-final-v5.3`, y caratulas | Claude (Anthropic) | Creacion de las etiquetas anotadas sobre el commit entregado y regeneracion de la caratula con `generar_caratula.py` | Ninguna etiqueta publicada se movio ni se borro; cada una se comprobo en el remoto. **La decision de cual es la vigente es del equipo** |
| Registro integro de la generacion del Conjunto A | Claude (Anthropic) | Localizacion de la conversacion de la generacion en la exportacion de datos de claude.ai del analista lider, comprobacion de que los 26 requisitos generados son los depositados, y composicion de `registro_generacion_conjunto_A.md` con las omisiones que declara: razonamiento interno, texto de EV-15, busquedas en archivos del proyecto y el script de la tabla de desciego. Correccion de los documentos que describian la generacion como una consigna literal con temperatura controlada | **La exportacion la descargo el analista lider desde su cuenta.** Las demas conversaciones se buscaron por texto de forma automatica, y de ellas solo se mostraron el titulo y las fechas. El extracto lleva su SHA-256 para que el original pueda contrastarse |
| Declaracion de los integrantes retirados | Claude (Anthropic) | Redaccion uniforme, en README, `composicion_equipo.md`, el ERS, el registro OSF, el banco de preguntas y el README de `10_Autoria`, de que Mendoza Palma y Gilces Carranza estan retirados del equipo; y correccion de la linea de autores del folleto de la defensa, que aun nombraba a los cinco | **La situacion de cada integrante la declaro el analista lider.** Se comprobo contra el historial que ninguno de los dos tiene confirmaciones. No se tocaron los documentos firmados de fechas anteriores, que reflejan la composicion de su momento |
...
| Manuscrito: desbordes de linea y apartado *Use of AI-assisted technologies* | Claude (Anthropic) | Localizacion de los desbordes y correccion de su maquetacion; deteccion de que el apartado afirmaba que el Conjunto A se genero en condiciones controladas con temperatura, top-p y semilla registrados, y redaccion del parrafo que lo sustituye | La afirmacion contradecia `06_Experimento/prompts_llm/registro_generacion_conjunto_A.md`, la seccion 1 de esta declaracion y la desviacion 6: la interfaz de chat no expone esos parametros. El parrafo nuevo solo recoge lo que ya constaba en esos documentos. Recompilado en 15 paginas, sin errores ni desbordes |
| Verificacion previa y aporte individual firmados; `generar_aporte_individual.py` | Claude (Anthropic) | Ejecucion de `verificacion_previa.py --clonar` sobre `8d0a321`, conversion a PDF para firmar, correccion del generador del aporte individual, que descartaba el ultimo commit existente creyendolo el del deposito, y union de cada hoja firmada escaneada a su documento | Las diez comprobaciones automaticas pasaron sobre un clon limpio del remoto; las dos manuales las hicieron los firmantes. El aporte cuenta 173, 114 y 12 commits, lo mismo que `git shortlog -sne` sobre el remoto al imprimirlo. **Firmaron Cedeno Avila, Munoz Quinonez y Sanchez Cornejo el 2026-09-14** |
| Materiales de la defensa (`08_Defensa`) y linea base `2B-final-v5.9` | Claude (Anthropic) | Auditoria de todo el repositorio en busca de cifras retiradas; sustitucion en PowerPoint del titulo, el texto y la figura de la diapositiva 7 y de la URL de la 15; correccion del guion y del libreto; correccion sobre el PDF del folleto, que no tiene fuente; regeneracion de `guion.pdf` con Word desde su fuente; portada, historial de versiones del ERS, README, reporte y caratula para la nueva etiqueta | Cada cifra del folleto se tomo de `07_Datos/resultados/estadisticos/efectos.csv` y la figura es la que produce la orden unica; el DOI de Zenodo y el track de REFSQ, de `CITATION.cff`, de la propia presentacion y de `07_Publicacion/analisis_revistas.md`. Cada documento se reviso visualmente tras el cambio. **Decidio corregirlos Munoz Quinonez, Yeranick Esther**, que solicito tambien volver a verificar y a firmar. La verificacion previa se repitio sobre un clon limpio de `12ffaa6`, con las diez comprobaciones automaticas en verde, y el aporte individual cuenta 173, 119 y 12 commits, lo mismo que el remoto al imprimirlo. **Ambos los firmaron de nuevo, a mano, Cedeno Avila, Munoz Quinonez y Sanchez Cornejo el 2026-09-14**; no se reutilizo ninguna firma de la version anterior |
| Trazabilidad en el reporte y en la auditoria de calidad; linea base `2B-final-v5.10` | Claude (Anthropic) | Auditoria carpeta por carpeta contra la guia de cierre, recalculo de las submetricas de trazabilidad y redaccion de los parrafos, la tabla y la fila de retrospectiva corregidos | Las cifras se recalcularon sobre `04_Trazabilidad/matriz_trazabilidad.csv` y coinciden con `huerfanos_y_cadenas_rotas.md` y con `verificar_matriz.py`; las mediciones de agosto se conservan fechadas. En la misma auditoria se contrasto cada cifra de los documentos con su fuente y se corrigieron las grabaciones del reporte, contra `fichas_tecnicas.csv`; la referencia rota a `correspondencia_salidas.csv`; el numero de registros de EV-17, contado en `respuestas_cuestionario.csv`; y dos recuentos de filas, contados en la matriz y en el export del tablero. Se comprobo por script que existen todas las rutas citadas en los documentos LaTeX y todos los enlaces relativos de los Markdown, y se corrigieron las dos que apuntaban a carpetas trasladadas. La verificacion previa se repitio sobre un clon limpio de `08878c5`, con las diez comprobaciones automaticas en verde, y el aporte individual cuenta 173, 127 y 12 commits, lo mismo que el remoto al imprimirlo. **Ambos los firmaron de nuevo, a mano, Cedeno Avila, Munoz Quinonez y Sanchez Cornejo el 2026-09-15**; se comprobo que ninguna firma reutiliza las de las rondas anteriores Reporte recompilado sin errores ni desbordes. **Decidio corregirlo Munoz Quinonez, Yeranick Esther**, con nueva verificacion previa y nuevas firmas |
...
*Precision anadida el 2026-09-16:* los cambios de este tramo, como los del 14 de septiembre, los
preparo Sanchez Cornejo, Gary Alberto, con Claude en su computadora, en una carpeta por
confirmacion; Munoz Quinonez, Yeranick Esther, los reviso y decidio aplicarlos y subirlos.

### Trabajo del 16 de septiembre de 2026

Atiende el informe del examen suspenso del 2026-09-16 sobre `2B-final-v5.10`: apartados 3, 15 y 16.

| Seccion o artefacto | Herramienta | Tipo de asistencia | Metodo de validacion aplicado |
|---|---|---|---|
| `generar_tablas.py`, `analisis_por_item.py`, sus tablas, `diccionario_datos.csv`, `manuscrito_final.tex` y `reporte.tex` (apartado 16) | Claude (Anthropic) | Codigo que imprime cada kappa con su intervalo y que anade las columnas de pares e interpretabilidad; redaccion de la desviacion 4 del manuscrito sin la cifra retirada y de los parrafos del acuerdo con su intervalo | Cada kappa de la tabla coincide con `acuerdo_interevaluador_ic.csv`, y la tabla por requisito, con `efectos.csv`, sin ninguna discrepancia. La orden unica y `replicar.py` terminan sin errores y sin cambiar ninguna salida; las 15 afirmaciones coinciden. La busqueda de la cifra retirada da cero en la fuente y en el PDF. Manuscrito (15 paginas) y reporte recompilados sin errores |
| `10_Autoria/generar_fuentes_editables.py`, `fuentes_editables.md` y `10_Autoria/README.md` (apartado 15) | Claude (Anthropic) | Script que empareja cada fuente con su imagen y redaccion de las descripciones de A1, A3 y A10 | El script regenera los dos archivos sin cambios y las 134 rutas de la tabla existen. Las 20 sesiones de elicitacion de la bitacora son 6 con nota, cuyo archivo existe, y 14 con motivo, las mismas de `sesiones_sin_nota.csv` |
| `10_Autoria/retrospectiva_equipo.md` (apartado 16) | Claude (Anthropic) | Borrador con los hechos fechados y comprobados, y paso a limpio de las respuestas que **Munoz Quinonez, Yeranick Esther** dio por escrito sobre los reclamos, la ayuda recibida, lo aprendido y lo que haran distinto | Los hechos se contrastaron con el historial, las etiquetas y los informes del docente. Lo aprendido y lo que haran distinto son palabras suyas; ella decidio retirar dos ejemplos que no coincidian con el repositorio y reviso el texto antes de confirmarlo |
| Cierre: portada e historial del ERS, `README.md`, `CHANGELOG.md`, reporte, bitacora, caratula y manifiestos para `2B-final-v5.11` | Claude (Anthropic) | Redaccion de las entradas y regeneracion con los scripts del repositorio | ERS y reporte recompilados sin errores; el reporte pasa a 29 paginas por las filas nuevas de la tabla de observaciones. Manifiestos regenerados y comprobados sin fallos |
| Auditoria posterior a `2B-final-v5.11` y linea base `2B-final-v5.12` | Claude (Anthropic) | Auditoria del remoto contra el informe sobre un clon limpio: sumas, orden unica, compilacion del reporte y del manuscrito, verificacion previa, enlaces y coherencia de cifras entre documentos. Deteccion de que la amenaza T2 del manuscrito daba dos kappa del panel ampliado sin intervalo y atribuidos a diez evaluadores, y redaccion de la correccion; intervalo del kappa de Fleiss en la discusion y las conclusiones del reporte y en el banco de preguntas; nota de A1 sobre el commit de cierre | Las medias se comprobaron en `panel_ampliado/resultados/acuerdo_dos_vueltas.csv` (−0,0082 y −0,026, siete evaluadores) y los limites en `acuerdo_interevaluador_ic.csv` (0,143 a 0,480). Manuscrito recompilado en 15 paginas y reporte en 29, sin errores; las 15 afirmaciones coinciden. **Decidio subir la correccion y crear la nueva etiqueta Munoz Quinonez, Yeranick Esther** |
| Segunda auditoria, sobre `2B-final-v5.12`, y linea base `2B-final-v5.13` | Claude (Anthropic) | Auditoria del remoto sobre un clon limpio contra el informe y la guia: etiqueta, tabla fuente-imagen, bitacora, notas de campo, manuscrito, tablas, retrospectiva, sumas, orden unica, verificadores del repositorio y compilacion del ERS, del reporte y del manuscrito. Deteccion de que la retrospectiva era anterior a la ultima correccion (`15cfc93`); redaccion de las dos filas de su cronologia y de la fila de quien hizo que que la completan; coma del `README.md`; cierre | ERS (136 paginas, una mas por la fila nueva del historial), reporte (29) y manuscrito (15) recompilados sin errores y con el mismo texto que lo depositado; 66 comprobaciones sin fallos. Las filas nuevas de la retrospectiva solo recogen hechos que constan en el historial y en las etiquetas. **Munoz Quinonez, Yeranick Esther** pidio las correcciones y autorizo el cierre, la etiqueta y el envio |
| Tercera auditoria, sobre `2B-final-v5.13`, deposito de las firmas y linea base `2B-final-v5.14` | Claude (Anthropic) | Auditoria del remoto sobre un clon limpio contra el informe del examen suspenso: los cuatro pendientes, los criterios de piso, las sumas, la orden unica y la recompilacion del ERS, el reporte y el manuscrito. Ejecucion de `verificacion_previa.py --clonar` y de `generar_aporte_individual.py` sobre `e3857ca`, conversion de los dos documentos a PDF para imprimirlos, y montaje del PDF depositado con las paginas generadas mas la pagina de firmas escaneada. Redaccion de las entradas del cierre | 66 comprobaciones sin fallos; ERS (136 paginas), reporte (29) y manuscrito (15) con el mismo texto que lo depositado. Las firmas y sus fechas son de puno y letra de los tres integrantes, sobre el documento impreso; ninguna se genero ni se copio. **Munoz Quinonez, Yeranick Esther** entrego los escaneos y autorizo el deposito, el cierre, la etiqueta y el envio |
| **Operaciones de Git de este tramo** | Claude (Anthropic) | **Los cinco primeros cambios los preparo Sanchez Cornejo, Gary Alberto, con Claude en su computadora. Las confirmaciones, el envio al remoto y la etiqueta los ejecuto Claude desde el equipo de Munoz Quinonez, Yeranick Esther, a peticion expresa suya, con su identidad y bajo su supervision** | Cada cambio preparado se comprobo con `verificar.ps1` contra el arbol esperado, y cada confirmacion se reviso antes de hacerla. El envio al remoto y la etiqueta, solo con su autorizacion expresa |
