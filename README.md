...
| **Linea base vigente** | Etiqueta anotada **`2B-final-v5.14`**, la **unica vigente**: la version entregada tras la evaluacion de la Entrega Final, la guia de cierre del 2026-09-14 y el informe del examen suspenso del 2026-09-16: tamanos del efecto con el requisito como unidad (desviacion 7) y con pares efectivos e interpretabilidad declarados, cadena de analisis ejecutandose dentro de `07_Datos` (tambien como `run_all.py`), de donde el reporte, el manuscrito y el ERS toman sus tablas y figuras, notas de campo y sesiones de elicitacion en la bitacora, observaciones del docente con su resolucion en el reporte, materiales de la defensa con los efectos vigentes, verificacion previa y aporte individual firmados, las cifras vigentes de trazabilidad en el reporte y la auditoria de calidad, la URL del nuevo propietario del repositorio, los kappa con su intervalo de confianza en las tablas y en el texto, la tabla que empareja cada fuente editable con su imagen, la retrospectiva del cierre ([`10_Autoria/retrospectiva_equipo.md`](10_Autoria/retrospectiva_equipo.md)) y ningun kappa sin su intervalo en el texto, con la retrospectiva fechada despues de la ultima correccion, y la verificacion previa y el aporte individual firmados por los tres integrantes el 2026-09-17 sobre `e3857ca`. Esta sobre el ultimo commit de la rama `main`. **Quien revise este repositorio debe ir directamente a esa etiqueta** (`git checkout 2B-final-v5.14`). Sustituye a `2B-final-v5.13`. `2B-final`, `2B-final-v2.1`, `2B-final-v3.0`, `2B-final-v4.0` y `2B-final-v5.0` a `2B-final-v5.13` se conservan **solo como referencia historica** de lo que se califico o deposito en cada fecha; ninguna es linea base. La caratula nombra el commit anterior al que la deposita, porque un archivo no puede contener el identificador del commit que lo contiene |
...
| Especificacion de requisitos | [`01_ERS/ERS_SRS_2B_v2.0.pdf`](01_ERS/ERS_SRS_2B_v2.0.pdf) --- 136 paginas en A4, regeneradas desde el `.tex` con el ciclo completo de `bibtex`: sin referencias sin resolver y **sin ningun desborde** horizontal ni vertical en el registro de compilacion |
...
Paquetes requeridos, tal como los carga cada documento:

- `reporte.tex`: `inputenc`, `fontenc`, `babel`, `geometry`, `graphicx`, `float`, `longtable`,
  `booktabs`, `amsmath`, `amssymb`, `xcolor`, `enumitem`, `fancyhdr`, `lastpage`, `hyperref` y
  `csquotes`.
- `01_ERS/ERS_SRS_2B_v2.0.tex`: `inputenc`, `babel`, `geometry`, `graphicx`, `float`,
  `longtable`, `array`, `booktabs`, `amssymb`, `xcolor`, `hyperref`, `enumitem`, `fancyhdr`,
  `titlesec` y `lastpage`.
